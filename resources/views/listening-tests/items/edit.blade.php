...
@section('title', 'Listening Test Item Tahrirlash')
...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Item Tahrirlash</h1>
        ...
        @php
            ...
            $itemTitle = old('item_title', $bodyData['title'] ?? '');
            $itemContent = old('item_content', $bodyData['content'] ?? '');
            $itemOptions = old('item_options', $bodyData['options'] ?? '');
        @endphp

        <form action="{{ route('listening-tests.items.update', [$listeningTest, $item]) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    <option value="audio" {{ old('type', $item->type) == 'audio' ? 'selected' : '' }}>Audio</option>
                    <option value="question" {{ old('type', $item->type) == 'question' ? 'selected' : '' }}>Question</option>
                    <option value="instruction" {{ old('type', $item->type) == 'instruction' ? 'selected' : '' }}>Instruction</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="item_title" id="item_title" value="{{ $itemTitle }}" 
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
            ...
            <div class="mb-4">
                ...
                <textarea name="item_content" id="item_content" rows="4" 
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                          placeholder="Item mazmuni...">{{ $itemContent }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="item_options" id="item_options" rows="3" 
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                          placeholder="Qo'shimcha parametrlar (JSON format)...">{{ $itemOptions }}</textarea>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mt-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Hozirgi Ma'lumotlar</h3>
