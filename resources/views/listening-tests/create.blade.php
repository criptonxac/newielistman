...
@section('title', 'Yangi Listening Test yaratish')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Yangi Listening Test yaratish</h1>
            <a href="{{ route('listening-tests.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded">
                Orqaga
            </a>
        </div>

        <form action="{{ route('listening-tests.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" name="audio_title" id="audio_title" value="{{ old('audio_title') }}" 
            ...
            <!-- Audio File Upload -->
            <div class="mb-4">
                <label for="audio_file" class="block text-gray-700 text-sm font-bold mb-2">Audio Fayl Yuklash:</label>
                <input type="file" name="audio_file" id="audio_file" accept=".mp3,.wav,.ogg,.m4a" 
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                <p class="text-xs text-gray-500 mt-1">MP3, WAV, OGG, M4A formatlarida, maksimal 20MB</p>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="audio_description" id="audio_description" rows="4" 
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                          placeholder="Audio haqida qisqacha ma'lumot...">{{ old('audio_description') }}</textarea>
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('listening-tests.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">
                    Bekor qilish
                </a>
            </div>
        </form>
